...
func (g *Generator) generate() {
	...
	buf.WriteString("#cgo CFLAGS: -I${SRCDIR}\n")
	buf.WriteString("#cgo LDFLAGS: -L${SRCDIR}/sdk/lib/release -ldiscord_partner_sdk\n")
	buf.WriteString("#include \"include/cdiscord.h\"\n#include <stdlib.h>\n\n")
	...
	for sig, cb := range uniqueSigs {
		...
		for i, arg := range cb.Args {
			...
			goArgType := ""
			if arg.CType == "void*" || arg.CType == "void *" {
				goArgType = "unsafe.Pointer"
			} else if strings.HasSuffix(arg.CType, "*") {
				inner := strings.TrimSuffix(arg.CType, "*")
				inner = strings.TrimSpace(inner)
				goArgType = "*C." + inner
			} else {
				goArgType = "C." + arg.CType
			}
		...
		if cb.ReturnType != "" {
			...
			if cb.CReturnType == "void*" || cb.CReturnType == "void *" {
		...
		if userDataVar != "" {
			...
			for _, arg := range cb.Args {
				...
				first = false
				if arg.Type == "string" {
					buf.WriteString(fmt.Sprintf("discordStringToString(%s)", arg.Name))
				} else if arg.IsBool {
					buf.WriteString(fmt.Sprintf("bool(%s)", arg.Name))
				} else if g.isKnownStruct(arg.Type) {
					buf.WriteString(fmt.Sprintf("*(*%s)(unsafe.Pointer(&%s))", arg.Type, arg.Name))
				} else if strings.HasPrefix(arg.Type, "*") {
					inner := strings.TrimPrefix(arg.Type, "*")
					buf.WriteString(fmt.Sprintf("(*%s)(unsafe.Pointer(%s))", inner, arg.Name))
				} else if strings.HasPrefix(arg.Type, "[]") {
					buf.WriteString(fmt.Sprintf("%sToSlice(%s)", arg.CType, arg.Name))
				} else {
					buf.WriteString(fmt.Sprintf("%s(%s)", arg.Type, arg.Name))
				}
			}
			buf.WriteString(")\n")
			if cb.ReturnType != "" {
	...
	for _, f := range g.functions {
		...
		if f.OutArg != nil {
			...
			} else if strings.HasPrefix(f.OutArg.Type, "[]") {
				buf.WriteString(fmt.Sprintf("\tvar res_out C.%s\n", f.OutArg.CType))
			} else {
				buf.WriteString(fmt.Sprintf("\tvar res_out C.%s\n", f.OutArg.CType))
			}
		...
		for _, arg := range f.Args {
			...
			if strings.Contains(arg.Name, "userDataFree") {
				callArgs = append(callArgs, "(C.Discord_FreeFn)(unsafe.Pointer(C.gateway_Free))")
				continue
			}
			if strings.Contains(arg.Name, "userData") {
				var targetCb *Arg
				for _, a := range f.Args {
					if cb, ok := g.callbacks[a.Type]; ok {
						...
						for _, cbArg := range cb.Args {
							if strings.Contains(cbArg.Name, "userData") {
								continue
							}
							if !first {
								buf.WriteString(", ")
							...
							first = false
							buf.WriteString(fmt.Sprintf("%s %s", cbArg.Name, cbArg.Type))
						}
						buf.WriteString(")")
						if cb.ReturnType != "" {
							buf.WriteString(" " + cb.ReturnType)
			...
			if cb, ok := g.callbacks[arg.Type]; ok {
				callArgs = append(callArgs, fmt.Sprintf("(C.%s)(unsafe.Pointer(C.gateway_%s))", arg.Type, cb.Signature))
				continue
			}
			if arg.Type == "string" {
	...
	err = os.WriteFile("discord.go", formatted, 0644)
	if err != nil {
		log.Fatalf("Failed to write discord.go: %v", err)
	}
	fmt.Println("Generated discord.go")
	err = os.WriteFile("gateway.c", cBuf.Bytes(), 0644)
	if err != nil {
		log.Fatalf("Failed to write gateway.c: %v", err)
	}
